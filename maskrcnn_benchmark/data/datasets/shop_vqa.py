...
class SHOP_VQA(data.Dataset):
    ...
    def __init__(self, path, transforms=None, split='train'):
        ...
        annotation_file = os.path.join(path, 'scenes', 'SHOP_VQA_' + split + '_scenes.json')
        print(annotation_file)
        print(os.listdir())
        assert os.path.isfile(annotation_file), 'Annotation file not found'
        objects_to_nums_file = os.path.join(path, 'SHOP_VQA_obj_name_to_num.json')
...
class SHOP_VQA_mask(SHOP_VQA):
    # Initialise dataset from either original file or preprocessed for detection
    def __init__(self, path, transforms=None, split='train'):
        super(SHOP_VQA_mask, self).__init__(path, transforms=transforms, split=split)
        assert split in ['train', 'val', 'test', 'benchmark']
        preprocessed_file = os.path.join(
            path, 'scenes', 'SHOP_VQA_' + split + '_scenes_maskrcnn.json')
    ...
    def __getitem__(self, idx):
        image, annotation = super(SHOP_VQA_mask, self).__getitem__(idx)
        # print(annotation)
